import { e as createComponent, f as createAstro, k as renderComponent, r as renderTemplate, m as maybeRenderHead, h as addAttribute } from '../chunks/astro/server_B7r4Bv6B.mjs';
import 'piccolore';
import { $ as $$Layout } from '../chunks/Layout_BZZLFtxH.mjs';
import { g as getMisas } from '../chunks/misas_wKuj3Yx2.mjs';
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { success, data: misas } = await getMisas(token);
...
Misas
</h1> ${token && renderTemplate`<a href="/misas/add" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition">
+ Nueva Misa
...
📅 ${formatDate(misa.dateMisa)} </p> </a>`)} </div> </section>`} ${(!misas || misas.length === 0) && renderTemplate`<section class="flex flex-col items-center justify-center h-full gap-3.5"> <p class="text-center text-gray-500">
No hay misas registradas.
</p> ${!token && renderTemplate`<p class="text-center text-gray-500">
