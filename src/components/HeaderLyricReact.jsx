...
export const HeaderLyricReact = ({ title, artist, tone, category, user }) => {
    return (
        <header className="mb-8">
            <h1 className="text-4xl font-extrabold text-text-main mb-1">
                {title || "Título Desconocido"}
            </h1>
            <h2 className="text-xl text-accent-main font-medium">
                <span className="text-text-secondary">Autor:</span>{" "}
                <span>{artist || "Artista Desconocido"}</span>
            </h2>
            <div className="flex flex-col sm:flex-row sm:justify-start justify-between gap-4 mt-4 text-sm text-text-secondary">
                <div className="flex items-center justify-start gap-2">
                    <span className="bg-bg-secondary px-3 py-1 rounded-full border border-white/10">
                    ...
                    {category && (
                        <a href={`/search/all?categoryId=${category.id}`} target="_blank" rel="noreferrer">
                            <span className="cursor-pointer hover:underline bg-bg-secondary px-3 py-1 rounded-full border border-white/10 text-accent-main/90 font-bold">
                                {category.name}
                            </span>
                        </a>
                    )}
